...
`rgb/image_raw`(640×480 bgr8)는 1프레임 ~900KB → 30fps 기준 ~216Mbps.  
FastDDS가 이를 UDP 600개 이상으로 단편화해서 전송하며, WiFi에서 단편 1개만 손실돼도 프레임 전체가 드롭된다.  
**WiFi 환경에서는 `rgb/image_raw` 대신 `rgb/image_raw/compressed` 사용 권장.**

→ 증상·진단·해결 상세: [ros2_image_raw_message_lost_wifi.md](../network/ros2_image_raw_message_lost_wifi.md)
