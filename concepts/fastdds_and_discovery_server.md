...
```
IP   = 건물 주소  (어떤 컴퓨터인지)
Port = 호실 번호  (그 컴퓨터에서 어떤 프로그램인지)
```

하나의 컴퓨터에서 여러 프로그램이 동시에 네트워크를 사용할 수 있다:

```
<IP> : 22    → SSH 서버 (원격 터미널)
<IP> : 80    → 웹 서버
<IP> : 11811 → FastDDS Discovery Server
```

**IP가 다르면 포트가 같아도 완전히 다른 서버다:**

```
<Robot1_IP> : 11811  ← Robot1의 Discovery Server
<Robot2_IP> : 11811  ← Robot2의 Discovery Server
```

→ 포트가 둘 다 11811이어도 IP가 다르니 별개. 이래서 "포트는 같아도 된다"가 성립.

> 검증: TCP/IP 표준 (RFC 793) — 소켓은 IP+Port 조합으로 식별

---

## SSH란

다른 컴퓨터의 터미널을 내 화면에서 여는 도구. 로봇(Raspberry Pi)은 모니터·키보드가 없으므로 SSH로 접속해서 설정하거나 명령을 실행한다.

```bash
ssh ubuntu@<로봇IP>
#    ↑       ↑
#   계정명   접속할 컴퓨터 IP (22번 포트 자동 사용)
```

접속 후 프롬프트가 바뀌며, 이후 입력하는 명령은 전부 원격 컴퓨터에서 실행된다.  
`exit`로 원래 터미널로 복귀.

> 검증: OpenSSH 공식 문서 — 기본 포트 22, Secure Shell 프로토콜
